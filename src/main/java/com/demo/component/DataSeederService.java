...
/**
 * Separated bean so @Transactional(REQUIRES_NEW) works properly via Spring proxy.
 */
@Slf4j
@Component
...
public class DataSeederService {
    ...
    public void seed() {
        if (roleRepository.count() > 0) {
            log.info("DataSeeder: roles already exist, skipping.");
            return;
        }

        log.info("DataSeeder: seeding roles and permissions...");
        ...
        if (!usersRepository.existsByUsername("admin")) {
            usersRepository.save(Users.builder()
                    ...
                    .enabled(true)
                    .build());
            log.info("DataSeeder: created admin user (username=admin, password=admin123)");
        }
